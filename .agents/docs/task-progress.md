...
- [x] **Subaccount Name Display** — Proactively fetch and store the human-readable name during OAuth and show it in the UI footer.
- [x] **Per-location API Keys** — Store PayMongo secret and publishable keys per location in the DB instead of relying solely on `.env`.
- [x] **Dynamic Webhook Sideloading** — Automatically create/recreate webhooks on PayMongo side for each location upon connection to capture unique secrets.
- [x] **Robust Provider Config UI** — Professional landscape layout with per-location key management, readonly protection, and live/test side-by-side columns.
- [x] **Provider config error detail display** — Show GHL API error details on failure, not just generic messages.
- [x] **GHL API Versioning Compliance** — Added required `Version` headers to all GHL API requests.
...
- [x] PayMongo webhook signature verification — Robustly validates `li` (live) and `te` (test) signatures against per-location secrets with global fallbacks.
...
- [x] **Cloud SQL Setup** — Set up managed MySQL for transaction data
