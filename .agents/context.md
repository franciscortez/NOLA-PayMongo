...
| Tunneling       | ngrok (for HTTPS in development)               |
...
GHL loads iFrame → /checkout (CheckoutController::show)
  ...
  → JS posts to /checkout/create-session
  → CheckoutController creates PayMongo Checkout Session
...
PayMongo sends webhook to /api/webhook/paymongo
  → PayMongoWebhookController handles:
  ...
  → On paid: GhlWebhookService sends payment.captured event to GHL
```
...
3. **Token refresh**: `GhlService::refreshToken()` exists but is not automatically called on expiry
4. **`list_payment_methods` and `charge_payment`**: Placeholder only — not yet implemented (card vaulting)
5. **Provider config uses `.env` keys**: PayMongo keys are pushed from server env, not user-input
